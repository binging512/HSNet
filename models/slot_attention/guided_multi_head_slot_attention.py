...
class GuidedMultiHeadSlotAttention(Module):
    def __init__(
        self,
        num_slots,
        dim,
        ...
        query_layer = nn.TransformerDecoderLayer(dim, nhead=8, dim_feedforward=dim*2, batch_first=True)
        self.query_proj = nn.TransformerDecoder(query_layer, 2)
    ...
    def forward(
        ...
        b, n, d, device, dtype = *key_value.shape, key_value.device, key_value.dtype
        ...
        query = self.query_proj(slots, query)

        key_value = self.norm_input(key_value)

        k, v = self.to_k(key_value), self.to_v(key_value)
