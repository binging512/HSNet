...
from .slot_attention import MultiHeadSlotAttention, GuidedMultiHeadSlotAttention, AdaptiveSlotWrapper
...
class Hier_OT_Slot_Attention(nn.Module):
    def __init__(self, d_model, dropout=0.25, ot_reg=0.1, ot_tau=0.5, ot_impl="pot-uot-l2"):
        super(Hier_OT_Slot_Attention, self).__init__()
        # Path slot attn
        path_slot_layer_1 = MultiHeadSlotAttention(num_slots=32, dim=d_model, heads = 4, dim_head = 64, iters = 3, eps = 1e-8, hidden_dim = 256)
        self.path_slot_attn_1 = AdaptiveSlotWrapper(path_slot_layer_1, temperature=0.5)
        path_slot_layer_2 = MultiHeadSlotAttention(num_slots=16, dim=d_model, heads = 4, dim_head = 64, iters = 3, eps = 1e-8, hidden_dim = 256)
        self.path_slot_attn_2 = AdaptiveSlotWrapper(path_slot_layer_2, temperature=0.5)
        path_slot_layer_3 = MultiHeadSlotAttention(num_slots=8, dim=d_model, heads = 4, dim_head = 64, iters = 3, eps = 1e-8, hidden_dim = 256)
        self.path_slot_attn_3 = AdaptiveSlotWrapper(path_slot_layer_3, temperature=0.5)

        self.path_omic_coattn_1 = OT_Attn_assem(impl=ot_impl,ot_reg=ot_reg,ot_tau=ot_tau)
        self.path_omic_coattn_2 = OT_Attn_assem(impl=ot_impl,ot_reg=ot_reg,ot_tau=ot_tau)
        self.path_omic_coattn_3 = OT_Attn_assem(impl=ot_impl,ot_reg=ot_reg,ot_tau=ot_tau)
        self.path_text_coattn_1 = OT_Attn_assem(impl=ot_impl,ot_reg=ot_reg,ot_tau=ot_tau)
        self.path_text_coattn_2 = OT_Attn_assem(impl=ot_impl,ot_reg=ot_reg,ot_tau=ot_tau)
        self.path_text_coattn_3 = OT_Attn_assem(impl=ot_impl,ot_reg=ot_reg,ot_tau=ot_tau)
        ...
        self.omic_rho = nn.Sequential(*[nn.Linear(d_model, d_model), nn.ReLU(), nn.Dropout(dropout)])
        ...
        self.path_omic_attention_head = Attn_Net_Gated(L=d_model, D=d_model, dropout=dropout, n_classes=1)
        self.path_omic_rho = nn.Sequential(*[nn.Linear(d_model, d_model), nn.ReLU(), nn.Dropout(dropout)])
        ...
        self.path_rho = nn.Sequential(*[nn.Linear(d_model, d_model), nn.ReLU(), nn.Dropout(dropout)])
        
        path_text_encoder_layer = nn.TransformerEncoderLayer(d_model=256, nhead=8, dim_feedforward=512, dropout=dropout, activation='relu')
        self.path_text_transformer = nn.TransformerEncoder(path_text_encoder_layer, num_layers=2)
        self.path_text_attention_head = Attn_Net_Gated(L=d_model, D=d_model, dropout=dropout, n_classes=1)
        self.path_text_rho = nn.Sequential(*[nn.Linear(d_model, d_model), nn.ReLU(), nn.Dropout(dropout)])
        
        text_encoder_layer = nn.TransformerEncoderLayer(d_model=256, nhead=8, dim_feedforward=512, dropout=dropout, activation='relu')
        self.text_transformer = nn.TransformerEncoder(text_encoder_layer, num_layers=2)
        self.text_attention_head = Attn_Net_Gated(L=d_model, D=d_model, dropout=dropout, n_classes=1)
        self.text_rho = nn.Sequential(*[nn.Linear(d_model, d_model), nn.ReLU(), nn.Dropout(dropout)])
        
    def forward(self, h_path_bag, h_omic_bag, h_text_bag):
        h_path_slot_1, keep_path_slot_1 = self.path_slot_attn_1(h_path_bag.permute(1,0,2))
        h_path_slot_1 = h_path_slot_1.permute(1,0,2)    #32x1xC
        h_path_slot_2, keep_path_slot_2 = self.path_slot_attn_2(h_path_bag.permute(1,0,2))
        h_path_slot_2 = h_path_slot_2.permute(1,0,2)    #16x1xC
        h_path_slot_3, keep_path_slot_3 = self.path_slot_attn_3(h_path_bag.permute(1,0,2))
        h_path_slot_3 = h_path_slot_3.permute(1,0,2)    #8x1xC
        
        ### Coattn
        # Omic
        A_path_omic_coattn_1, _ = self.path_omic_coattn_1(h_path_slot_1, h_omic_bag)
        h_path_omic_coattn_1 = torch.mm(A_path_omic_coattn_1.squeeze(), h_path_slot_1.squeeze()).unsqueeze(1)     # 6x1xC
        A_path_omic_coattn_2, _ = self.path_omic_coattn_2(h_path_slot_2, h_omic_bag)
        h_path_omic_coattn_2 = torch.mm(A_path_omic_coattn_2.squeeze(), h_path_slot_2.squeeze()).unsqueeze(1)     # 6x1xC
        A_path_omic_coattn_3, _ = self.path_omic_coattn_3(h_path_slot_3, h_omic_bag)
        h_path_omic_coattn_3 = torch.mm(A_path_omic_coattn_3.squeeze(), h_path_slot_3.squeeze()).unsqueeze(1)     # 6x1xC
        # Text
        A_path_text_coattn_1, _ = self.path_text_coattn_1(h_path_slot_1, h_text_bag)
        h_path_text_coattn_1 = torch.mm(A_path_text_coattn_1.squeeze(), h_path_slot_1.squeeze()).unsqueeze(1)     # 3x1xC
        A_path_text_coattn_2, _ = self.path_text_coattn_2(h_path_slot_2, h_text_bag)
        h_path_text_coattn_2 = torch.mm(A_path_text_coattn_2.squeeze(), h_path_slot_2.squeeze()).unsqueeze(1)     # 3x1xC
        A_path_text_coattn_3, _ = self.path_text_coattn_3(h_path_slot_3, h_text_bag)
        h_path_text_coattn_3 = torch.mm(A_path_text_coattn_3.squeeze(), h_path_slot_3.squeeze()).unsqueeze(1)     # 3x1xC
        ...
        ### Path-Omic
        h_path_omic_coattn = torch.cat((h_path_omic_coattn_1,h_path_omic_coattn_2,h_path_omic_coattn_3), dim=0)
        ...
        h_path_omic = self.path_omic_rho(h_path_omic)
        
        ### Path
        h_path_coattn = torch.cat((h_path_slot_1,h_path_slot_2,h_path_slot_3), dim=0)
        ...
        ### Path-Text
        h_path_text_coattn = torch.cat((h_path_text_coattn_1,h_path_text_coattn_2,h_path_text_coattn_3), dim=0)
        ...
        h_path_text = self.path_text_rho(h_path_text)
        ...
        h_text = self.text_rho(h_text)
        
        attn_output = {
            "h_path_bag": h_path_bag,
            "h_omic_bag": h_omic_bag,
            "h_text_bag": h_text_bag,
            
            "h_omic":h_omic,
            "h_path_omic":h_omic,
            "h_path": h_path,
            "h_path_text": h_path_text,
            "h_text": h_text,
            
            'keep_path_slot_1': keep_path_slot_1,
            'keep_path_slot_2': keep_path_slot_2,
            'keep_path_slot_3': keep_path_slot_3,
        }
        return attn_output
...
class ROD_pro(nn.Module):
    ...
    def forward(self, h_path, h_path_omic, h_path_text):
        h_path_r = self.projector(h_path)
        h_path_o = h_path_r - h_path_omic - h_path_text
        h_path_f = h_path_r + h_path_o
        h_path_f = self.fusor(h_path_f)
...
###############################
### HSICFNet Implementation ###
###############################
class HSICFNet(nn.Module):
    def __init__(self, fusion='concat', omic_sizes=[100, 200, 300, 400, 500, 600], n_classes=4,
        ...
        self.interaction = Hier_OT_Slot_Attention(size[2], dropout, ot_impl=ot_impl,ot_reg=ot_reg,ot_tau=ot_tau)
    ...
    def forward(self, **kwargs):
        ...
        ### Interaction
        attn_output = self.interaction(h_path_bag, h_omic_bag, h_text_bag)
        h_omic = attn_output["h_omic"].squeeze()
        h_path_omic = attn_output["h_path_omic"].squeeze()
        h_path = attn_output["h_path"].squeeze()
        h_path_text = attn_output["h_path_text"].squeeze()
        h_text = attn_output["h_text"].squeeze()
