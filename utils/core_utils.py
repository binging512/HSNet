...
def train(datasets: tuple, cur: int, args: Namespace):
    ...
    if args.model_type =='snn':
        ...
        from models.model_hsicfnet import HSICFNet
        model_dict = {'ot_reg': args.ot_reg, 'ot_tau': args.ot_tau, 'ot_impl': args.ot_impl,'fusion': args.fusion, 'omic_sizes': args.omic_sizes, 'n_classes': args.n_classes}
        model = HSICFNet(**model_dict)
    else:
        raise NotImplementedError
        ...
        if args.mode == 'coattn':
            ...
            if args.model_type in ['motcat_text', 'motcat_text_only']:
                ...
                if args.scheduler in ['cosine']:
                    scheduler.step()
            elif args.model_type in ['hsicfnet','hsicfnet_demo','hsicfnet_treat','hsicfnet_wotensor']:
                from trainer.icf_trainer import train_loop_survival_icf, validate_survival_icf
                train_loop_survival_icf(epoch, args.bs_micro, model, train_loader, optimizer, scheduler, args.n_classes, writer, loss_fn_dict, reg_fn, args.lambda_reg, args.gc, args)
                val_latest, c_index_val, stop = validate_survival_icf(cur, epoch, args.bs_micro, model, val_loader, args.n_classes, early_stopping, monitor_cindex, writer, loss_fn_dict, reg_fn, args.lambda_reg, args.results_dir, args)
                if args.scheduler in ['cosine']:
                    scheduler.step()
            elif args.model_type in ['hgsicfnet']:
                from trainer.icf_trainer import train_loop_survival_icf, validate_survival_icf
                train_loop_survival_icf(epoch, args.bs_micro, model, train_loader, optimizer, scheduler, args.n_classes, writer, loss_fn_dict, reg_fn, args.lambda_reg, args.gc, args)
                val_latest, c_index_val, stop = validate_survival_icf(cur, epoch, args.bs_micro, model, val_loader, args.n_classes, early_stopping, monitor_cindex, writer, loss_fn_dict, reg_fn, args.lambda_reg, args.results_dir, args)
                if args.scheduler in ['cosine']:
